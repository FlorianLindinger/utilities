...
:: ============================================================================
:: Ultra-Small Nuitka Compilation Script (Standalone Folder + UPX)
...
set OUTPUT_NAME=create_icon_small_dist
set PYTHON_CMD=python

echo.
echo ============================================================================
echo Ultra-Small Nuitka Compilation (Standalone Folder + UPX)
echo ============================================================================
echo.
echo Script: %SCRIPT_NAME%
echo Output: %OUTPUT_NAME%.dist (Folder)
echo.
...
set FLAGS=--standalone
set FLAGS=%FLAGS% --lto=yes

:: Output control
set FLAGS=%FLAGS% --output-filename=%OUTPUT_NAME%
...
if exist "%OUTPUT_NAME%.dist" (
    set "FOLDER_SIZE=0"
    for /r "%OUTPUT_NAME%.dist" %%I in (*) do (
        set /a "FOLDER_SIZE+=%%~zI"
    )
    
    set /a "SIZE_KB=!FOLDER_SIZE! / 1024"
    set /a "SIZE_MB=!SIZE_KB! / 1024"
    
    echo Output folder: %OUTPUT_NAME%.dist
    echo Folder size:   !FOLDER_SIZE! bytes (!SIZE_KB! KB / !SIZE_MB! MB)
    ...
    if exist "create_icon_py_smal_oldl.exe" (
        for %%I in ("create_icon_py_smal_oldl.exe") do (
            ...
            set /a "REDUCTION=!OLD_SIZE! - !FOLDER_SIZE!"
