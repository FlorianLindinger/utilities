...
:: --- MAX OPTIMIZATION FLAGS FOR FASTEST EXECUTION ---
:: --onefile: Create a single compressed executable file
...
python -m nuitka ^
  ...
  --remove-output ^
  --output-dir=. ^
  --output-filename="%file_name%_py_fast.exe" ^
  "%python_file%"

:: print end message
echo.
if exist "%file_name%_py_fast.exe" (
    echo ============================================
    echo SUCCESS! Optimized EXE created: %file_name%_py_fast.exe
    echo ============================================
) else (
    echo ============================================
